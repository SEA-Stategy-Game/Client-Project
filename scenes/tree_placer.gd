...
func fill_from_reference(tree_data: Dictionary) -> void:
	if reference_layer == null:
		print("Reference layer not set!")
		return
	
	var pos = tree_data["position"]
	var ref_tile = reference_layer.get_cell(Vector2i(int(pos[0]), int(pos[1])))
	
	if ref_tile != -1:
		var tile_id = random_tiles[randi() % random_tiles.size()]
		set_cell(Vector2i(int(pos[0]), int(pos[1])), tile_id)
