...
func set_tile(x: int, y: int, terrain: int) -> void:
    ...
    if tile == null:
        tile = MapTileScript.new(x, y, MapTileScript.TerrainType.PLAINS, null)
...
func initialize_tiles() -> void:
    ...
    for y in range(height):
        for x in range(width):
            tiles[_index(x, y)] = MapTileScript.new(x, y, MapTileScript.TerrainType.PLAINS, null)
    ...
    for y in range(height):
        for x in range(width):
            ...
            var is_land = val > -0.4
            var terrain = MapTileScript.TerrainType.WATER

            if is_land:
                var t_v = noise.get_noise_2d(x, y)
                if t_v > 0.3:
                    terrain = MapTileScript.TerrainType.HILLS
                elif t_v > 0.05:
                    terrain = MapTileScript.TerrainType.FOREST
                elif t_v < -0.6:
                    terrain = MapTileScript.TerrainType.WATER
                else:
                    terrain = MapTileScript.TerrainType.PLAINS
    ...
    for y in range(height):
        for x in range(width):
            if x < border or y < border or x >= width - border or y >= height - border:
                set_tile(x, y, MapTileScript.TerrainType.WATER)